---
layout: post
title: A Geometric Derivation of Conjugate Gradient
tags: cg
author: Rajat Vadiraj Dwaraknath
---
  <h5 class="text-center mt-6 mb-6">Thanks also to Anjan Dwaraknath for helpful discussions.</h5>
...
We prove this statement by showing that the ${r_k}$ form an orthogonal basis for the Krylov sequence and then use the fact that this orthogonal basis must be unique upto scaling to get the result.

First, observe that $r_k \in K_{k+1}$. This is because $r_k = b - A x_k$ and $b \in \mathcal{K}\_1$ and $x_k \in \mathcal{K}\_k \implies A x_k \in \mathcal{K}_{k+1}$. Now, we can rewrite the residual as 
...
Therefore, ${r_i}$ is an orthogonal set of vectors and $\text{span}(r_0, \dots , r_{k-1}) = \mathcal{K}\_k$. This means that $r_k$ must be parallel to $q_{k+1}$ from Lanczos since we found an orthogonal basis for the Krylov sequence, and we can show by induction that this basis is unique upto scaling factors. We have therefore shown the required statement.
...
* $$p_{k+1} = r_{k} - \frac{p_k^T A r_k}{p_k^T A p_k} p_{k}$$
* $$x_{k+1} = x_{k} + \frac{p_{k+1}^T b}{p_{k+1}^T A p_{k+1}} p_{k+1}$$
